...
class Neo4jFeatureExtractor:
    ...
    def __init__(self) -> None:
        ...
        self.logger = ProjectLogger.get_logger("Neo4jFeatureExtractor")
        self.graph_name = "aml_graph"
    ...
    def _cleanup_existing_graph(self) -> None:
        ...
        exists = self.gds.graph.exists(self.graph_name).exists
        if exists:
            self.logger.info(f"Graph '{self.graph_name}' exists. Dropping it.")
            g = self.gds.graph.get(self.graph_name)
            g.drop()
                ...
                records.append(
                    {
                        ...
                        "wcc_id": record["wcc_id"],
                        "pagerank": record["pagerank"],
                        "fastrp_emb": record["fastrp_emb"],
                    }
                )
